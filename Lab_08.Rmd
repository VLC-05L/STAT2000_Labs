...
On completion of this lab you will be able to:
 ...
 - Apply simple linear regression using statistical software
...
In the Week 8 lecture we looked at testing for a linear relationship using the populaiton correclation coefficient. Simple linear regression was introduced. Testing for a linear relationhip, predictions, and intervals where covered. 


### Data Files {-}

```{r echo=FALSE}
xfun::embed_file("Lab_08_Stress.sav")
```  

```{r include=FALSE}
S_files = list.files(pattern='08_S_.*')
...
zip::zipr(zipfile = "Lab_08_Additional_Files.zip", files = S_files)
xfun::embed_file("Lab_08_Additional_Files.zip")
```  
...
We wish to investigate how well the Hopkins score could be predicted based on the stress measure. The data are provided in *Lab_08_Stress.sav*.

#### Obtain a scatter plot for this data. What does this plot suggest about the relationship between stress and symptoms?  {-}
...
#### Fit a least squares regression model. State the equation representing the fitted least squares regression model.  {-}
...
#### What is the coefficient of determination (R²) for the data? Interpret your result.  {-}
...
#### Plot the residuals against the predictor variable, stress. What information does this plot give you about the model? {-}
...
#### Plot the residuals against the predicted Y values. Explain how for simple linear regression this provides basically the same information as the previous plot. {-}
...
#### Analyse the distribution of the residuals. Why is this important? {-}
...
#### We now wish to test whether a linear relationship exists between stress and symptoms. What are the null and alternative hypotheses for this test? {-}
...
#### What is the F test statistic for this test? What is the t test statistic? What is the relationship between the t-test statistic and the F test statistic for this test? {-}
...
#### Give an appropriate conclusion about the existence of a linear relationship between stress and symptoms.   {-}
...
#### Sometimes it may useful to consider confidence intervals to report results. {-}

  i. Obtain a 95% confidence interval for $\beta_1$ (the true slope of the regression line).  
...
A confidence interval gives an estimate of the size of an effect (in this case the effect of stress due to life events) as well as an estimate of the error associated with that estimate. It can also be easily interpreted to indicate whether a particular variable is statistically significant or not at a particular significance level, $\alpha$. For example: if the $(1 - \alpha)$% confidence interval for $\beta_1$ does not contain 0, this is equivalent to stating that $\beta_1$ is statistically different from 0 at the 5% significance level. (i.e. rejecting $H_0:\beta_1 = 0$ at the 5% significance level.)
...
#### Determine a 95% confidence interval for the expected mean symptoms score for students with reported stress levels of 35?  {-}
...
#### Calculate a 95% prediction interval for the symptoms score of a student who reported a stress level of 35? Also compare this result with the 95% confidence interval.  {-}
