...
data <- haven::read_sav('Lab_05_Recall.sav')
...
```{r echo=FALSE}
xfun::embed_file("Lab_05_Recall.sav")
```
<br>
```{r echo=FALSE}
xfun::embed_file("Lab_05_Advertising.sav")
```  



```{r include=FALSE}
S_files = list.files(pattern='05_S_.*')
...
zip::zipr(zipfile = "Lab_05_Additional_Files.zip", files = S_files)
xfun::embed_file("Lab_05_Additional_Files.zip")
```  
...
Recall the example provided in Lecture 5:
 ...
 - The response variable is the number of words recalled. The data appears in *Lab_05_Recall.sav*.

Number of words recalled for given levels of mood during learning and at recall.

#### Express the two-way ANOVA model required for analysing this data algebraically (i.e., in terms of a mathematical equation). Clearly define all the symbols in the context of this example. {-}
...
#### Fit the appropriate ANOVA model. Record the means and obtain plots of the factor level (group) sample means for each factor to enable a visual consideration of the effects of the factors. {-}
...
#### Write the relevant null and alternative hypotheses for testing whether there is an interaction between the mood during learning and the mood during recall in how these factors relate to the number of words recalled. What is the relevant test statistic, p-value and your decision? {-}
...
#### Write the relevant null and alternative hypotheses for testing whether the mood during learning has an effect on the number of words recalled. What is the relevant test statistic, p-value and your decision? {-}
...
#### Write the relevant null and alternative hypotheses for testing whether the mood during recall has an effect on the number of words recalled. What is the relevant test statistic, p-value and your decision? {-}
...
#### Provide a concluding paragraph that would be appropriate to include in a report summarising the results of this research. {-}
...
#### Use this example to explain why is it important to always check for an interaction effect before interpreting the main effects. {-}
...
The results of the study are in *Lab_05_Advertising.sav*. If using R, remember to convert the predictor variables to factors using `as.factor()`.

```{r}
data <- haven::read_sav('Lab_05_Advertising.sav')
data <- as.data.frame(data)
...
#### The researcher has asked that you perform a statistical analysis to determine if the grade of celebrity and or gender of the spokesperson effects recall in television advertisements. As part of your analysis, be sure to: {-}

 - Include the null and alternative hypotheses for the elements of the model you are testing.
 - Justify your final decision.
 
This requires a two way ANOVA to be performed. 
...
#### You are now told that when individuals were allocated to groups, they were informed of the style of advertisements they were going to be eventually tested on. Does this information change the validity of any conclusions you are able to make from the experiment? {-}
...
## Learning Recall Revisted

### Assumptions and Remedial Measures

#### In Section 5.1, there was no question on testing the assumptions of the two-way ANOVA. Using the *Lab_05_Recall.sav* data set, test the assumptions when a two-way ANOVA is fitted. {-}

##### Solutions

We are not explicitly told about whether the individuals are randomly selected and or randomly allocated to treatment groups. Therefore randomness and independence may be violated. We should also test for equality of variance, normality and outliers. 
...
qqPlot(residuals(lm_1))

```

Levene's test shows no evidence (p = 0.81) of non-equal variance between the groups therefore this assumption is satisfied. A review of the Shapiro Wilk test of normality indicates that the assumption of normaility is violated (p = 0.048). We should therefore look at doing a transformation of the response variable. 

#### One of the assumptions should have failed. Transform the response variable and re-run the model using the new response variable.  {-}

##### Solutions

```{r, eval=TRUE}
data$WordsRecalled_Log = log(data$WordsRecalled)
lm_1_Log = lm(WordsRecalled_Log~LearningMood*RecallMood, data = data)
anova(lm_1_Log)            

leveneTest(lm_1_Log)

shapiro.test(residuals(lm_1_Log))
qqPlot(residuals(lm_1_Log))
```

Looking at the assumptions first, the normaility and equal variance assumption are both now satisfied. 

```{r, eval=TRUE}
anova(lm_1_Log)            
```

Looking at the ANOVA table now, the same factors are statistically significant in explaining the variation in the log of the number of words recalled. 
...
If you have time still, have a go at the next exercise. 
...
### Case Study: A patient by any other name...
...
#### Explain what the response and explanatory variables are in this study. How have they been measured or determined? {-}
...
#### How has the data been analysed? Is this method appropriate? {-}
...
#### Explain why the concept of interaction is so important in this study. {-}
...
#### Summarise the main results of the study. {-}
...
#### This study is a specific example of how people's judgement can be influenced or biased by their expectations. Why is this general concept often important when developing the detailed methodology for a research study? {-}
