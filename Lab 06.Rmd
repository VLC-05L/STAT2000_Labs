...
```{r echo=FALSE}
xfun::embed_file("Lab 06 - Accounting.csv")
```  


## 6.1 Random Effects Vs Fixed Effects

For the follwing scenarios, consider if you would regard the variable as a fixed effect or a random effect. 

#### 1. A large bread baking business with many production lines are interested in if the production line has an effect on bread quality. They take a random sample of production lines and collect some data. 
...
#### 2. A scientist is interested in how certain states of being effect ones driving ability. The states they choose to investigate are, *sleepy*, *drunk*, and *normal* as a control.  
...
#### 3. Track times at a racing circuit are of interest to an exotic car manufacturer. They want to know if the driver of a car and the type of car has an effect on the times. They randomly select 10 top level drivers and have them set several times in their own car and two of their competitors cars. Comment on the variables of `car` and `driver`.   
...
## 6.2 Random Effects Vs Fixed Effects
...
The data are available in *Lab 06 - Accounting.csv*.


#### 1. Explain why we would consider the variable `partner` to be a random effect. 
...
#### 2. Write down the model for the one-way random effects ANOVA model being sure to define all symbols and indexes.   
...
#### 3. Fit a one-way random effect model with partner being the random factor. Write your null and alternative hypothesis for testing whether `partner` has an affect on `write_off` and provide your conclusion. Based on your conclusion, provide a recomendation to the accounting firm. 
...
Where we used the `lm()` function for the fixed effects ANOVA, in the `lme4` package we use the `lmer()` function. This is used similarly to the `lm()` packag, however we define the random effects a little differently. To fit a random effect like partner we would use the following: 
```{r eval=F}
...
```{r }
data = read.csv('Lab 06 - Accounting.csv')
...
#### 4. Test the assumptions of you model above. 
...
#### 7. Why has our conclusion changed about the effect of `partner` when fitting our mixed effect ANOVA model? 
